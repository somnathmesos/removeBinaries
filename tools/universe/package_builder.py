...
class UniversePackageBuilder(object):
    ...
    def _get_nifi_mapping_for_content(self, orig_content):
        '''Returns a nifi mapping (dict) for the following cases:
        ...
        - Custom environment params like 'NIFI_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default nifi values (may be overridden via eg NIFI_PACKAGE_VERSION envvars):
        now = time.time()
        nifi_mapping = {
        ...
        # look for any 'sha256:filename' or 'sha256:filename@url' nifi params, and get shas for those.
        ...
        for sha_param in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            nifi_mapping['sha256:{}'.format(sha_param)] = sha_value

        # import any custom "NIFI_SOME_PARAM" environment variables as "some-param":
        for env_key, env_val in os.environ.items():
            if env_key.startswith('NIFI_'):
                # 'NIFI_SOME_KEY' => 'some-key'
                nifi_mapping[env_key[len('NIFI_'):].lower().replace('_', '-')] = env_val

        return nifi_mapping

    def _apply_templating_to_file(self, filename, orig_content):
        nifi_mapping = self._get_nifi_mapping_for_content(orig_content)
        new_content = orig_content
        for nifi_key, nifi_val in nifi_mapping.items():
            new_content = new_content.replace('{{%s}}' % nifi_key,
                                              nifi_val)
        ...
        nifi_keys = list(nifi_mapping.keys())
        nifi_keys.sort()
        for key in nifi_keys:
            logger.info('  {{%s}} => %s' % (key, nifi_mapping[key]))
