...
class UniversePackageBuilder(object):
    ...
    def _get_nifi_mapping_for_content(self, orig_content):
        ...
        # look for any 'sha256:filename' nifi params, and get shas for those.
        # this avoids calculating shas unless they're requested by the nifi.
        for shafilename in re.findall('{{sha256:(.+?)}}', orig_content):
            ...
            nifi_mapping['sha256:{}'.format(
                shafilename)] = self._calculate_sha256(shafilepath)

        # import any custom NIFI_SOME_PARAM environment variables:
        for env_key, env_val in os.environ.items():
            if env_key.startswith('NIFI_'):
                # 'NIFI_SOME_KEY' => 'some-key'
                nifi_mapping[env_key[len('NIFI_'):].lower().replace(
                    '_', '-')] = env_val
