...
def print_help(argv):
    logger.info('Syntax: {} <package-name> <nifi-package-dir> [artifact files ...]'.format(argv[0]))
    ...
    logger.info('In addition, environment variables named \'NIFI_SOME_PARAMETER\' will be inserted against the provided package nifi (with params of the form \'{{some-parameter}}\')')
...
def main(argv):
    ...
    # the package name:
    package_name = argv[1]
    # local path where the package nifi is located:
